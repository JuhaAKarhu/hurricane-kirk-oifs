...
def compute_geopotential_difference_anomaly(oifs_run, track_ds, timestamp, lat_center, lon_center,
    ...
    # Ribberink-style plotting geometry: ignore lat/lon metric differences
    # so the mask boundary coincides with the plotted circle in degree space.
    lon_grid, lat_grid = np.meshgrid(lon_subset, lat_subset)
    dlat_deg = lat_grid - lat_center
    dlon_deg = lon_grid - lon_center
    distance_grid = 111.0 * np.sqrt(dlat_deg**2 + dlon_deg**2)
...
def plot_geopotential_comparison():
    ...
    fig.suptitle('900-600 hPa Geopotential Height Difference Anomalies In the Control Run', fontsize=17.5, fontweight='bold')
    ...
    for idx, (data, params) in enumerate(zip(all_anom_data, all_params)):
        ...
        # Dashed separation line aligned with storm-motion direction
        # Extend through center to both sides of the 500 km circle
        sep_length_deg = 500.0 / 111.0
        ax.plot([lon_center - dlon_vel * sep_length_deg, lon_center + dlon_vel * sep_length_deg],
            [lat_center - dlat_vel * sep_length_deg, lat_center + dlat_vel * sep_length_deg],
    ...
    plt.subplots_adjust(wspace=0.15, hspace=0.3, left=0.06, right=0.87, bottom=0.30, top=0.86)
